...
async function joinGM(p){const seat=async()=>{await p.goto(BASE+"/join",{waitUntil:"domcontentloaded"});const s=p.locator('select[name="userid"]');await s.waitFor({state:"visible",timeout:30000});return s.locator("option").evaluateAll(o=>o.map(x=>({v:x.value,l:(x.textContent||"").trim(),d:x.disabled})).filter(x=>x.v&&/gamemaster/i.test(x.l)));};const us=await seat();const g=us.find(u=>!u.d)||us[0];if(!g)throw new Error("no gamemaster seat offered");for(const pw of [PW,""]){await p.evaluate(v=>{const el=document.querySelector('select[name="userid"]');el.value=v;el.dispatchEvent(new Event("change",{bubbles:true}));},g.v);await p.locator('input[name="password"]').fill(pw);await Promise.all([p.waitForNavigation({url:/\/game/,timeout:30000}).catch(()=>{}),p.locator('button[name="join"]').click()]);try{await p.waitForFunction(()=>window.game?.ready===true,undefined,{timeout:30000});return;}catch{await seat();}}throw new Error("could not join "+g.l);}
